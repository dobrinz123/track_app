...
describe('decodeVinResponse', () => {
  ...
  it('strips trailing 0x00 padding', () => {
    const pdu = Uint8Array.from([0x62, 0xf1, 0x90, ...asciiBytes('ABC123'), 0x00, 0x00, 0x00]);
    expect(decodeVinResponse(pdu)).toBe('ABC123');
  });

  it('strips trailing 0x20 (space) padding', () => {
    const pdu = Uint8Array.from([0x62, 0xf1, 0x90, ...asciiBytes('ABC123'), 0x20, 0x20]);
    expect(decodeVinResponse(pdu)).toBe('ABC123');
  });
