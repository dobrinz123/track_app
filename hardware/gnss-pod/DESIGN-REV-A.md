...
- **Rev A is USB-only (owner decision 2026-09-24).** J2 is NOT fitted on rev A units and no cell is connected in the car: the charger's hardware window cannot be tightened below a worst-case 51.5 °C hot trip (TI SLUS810N §9.3.6, §10.4), so a firmware-supervised cell behind the windscreen is not accepted for the first boards. The pod runs from USB through the BQ24073 power path (no cell needed; with TS open and TD low the charger simply never charges). Battery charging is exercised on the BENCH only, with §10A test 3, before a rev B decides between this firmware supervision and a hardware window comparator. The line below is the specification for that bench test and for rev B.
...
8. **Rev A never charges (no cell, CHG_EN held low, §6); rev B's plan is firmware supervision stopping at 40 °C cell temperature, with the BQ's TS trip (47.2 °C nominal, ≤ 51.5 °C worst case, §10.4) as the hardware backstop.** This is intended for safety. The pod runs from USB without charging. Operating limits are 85 °C for ESP32-S3-MINI-1 (N8 standard temp), SAM-M10Q and LSM6DSV16X. A windscreen in summer sun can exceed that. The enclosure (P2) needs shading and venting.
