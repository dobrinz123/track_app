...
| 3.5 | **Delete-all does not clear the stored VIN**, app settings, confirmed vehicle bindings, ruled-out signals, DID sweep records, or learned-circuit geometry | **TODO — release blocker** | The VIN is personal data and lives in `settings.lastSeenVin`. Until fixed, §3.3 and §7 of the privacy policy must keep the honest caveat they carry. Fix: extend `deleteAllStoredUserData()` to clear `settings` (preserving non-personal preferences if desired), `vehicle_profile_bindings`, `signal_finder_ruled_out`, `did_sweep_*`, and `learned_circuits` |
